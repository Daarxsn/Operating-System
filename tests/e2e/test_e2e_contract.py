...
for marker in (
    ...
    if marker not in runner:
        errors.append(
            f"7.10 runner missing acceptance marker/step: {marker}"
        )
...
for marker in (
    ...
    if marker not in validator:
        errors.append(
            f"repository validation pipeline missing runtime marker: {marker}"
        )


# 7.9 ABI validation must be explicitly executed by the end-to-end contract.
if not abi:
    errors.append(
        "7.9 ABI validation is not available to the end-to-end contract"
    )
else:
    try:
        result = subprocess.run(
            [sys.executable, str(ABI)],
            cwd=ROOT,
            capture_output=True,
            text=True,
            check=False,
        ...
        errors.append(
            f"7.9 ABI validation could not be executed: {exc}"
        )
    else:
        if result.returncode != 0:
            output = (result.stdout + result.stderr).strip()
            errors.append(
                "7.9 ABI validation failed during end-to-end contract validation"
            )
            if output:
...
if (
    "xyris-abi-compatibility-test" not in validator
    and "test_abi_compatibility.py" not in validator
):
    errors.append(
        "repository validation pipeline does not invoke 7.9 ABI validation"
    )


if "xyris-abi-v0.1" not in packager:
    errors.append(
        "XAPP packager does not declare the v0.1 ABI"
    )
...
for marker in (
    ...
    if marker.lower() not in doc.lower():
        errors.append(
            f"7.10 specification missing coverage: {marker}"
        )
